...
public class MatcherServer extends AbstractServer {
    ...
    public static class InputFormatter {
         ...
         */
        public Tuple<List<MatcherSample>, String> format(String input) {
            ...
            try {
                ...
                if (jsoninput instanceof JSONObject) {
                    ...
                    try {
                    	oldStateString = ((JSONObject) jsoninput).getString("oldState");
                    } catch (JSONException e) {}
//                    System.out.println(oldStateString);
                } else {
            ...
            return new Tuple<>(samples, oldStateString);
        }
    }
            ...
            return new ResponseHandler(request) {
                ...
                protected RESULT response(String request, StringBuilder response) {
                    try {
                        ...
                        sw.start();
                        Tuple<List<MatcherSample>, String> inputFormatResults = input.format(request);
                        final List<MatcherSample> samples = inputFormatResults.one();
                        final String oldStateString = inputFormatResults.two();
                        ...
                        scheduler.spawn(new Task() {
                            ...
                            public void run() {
                            	if(oldStateString == null)
                            		...
                            		try {
                            			MatcherKState oldState = new MatcherKState(new JSONObject(oldStateString), new MatcherFactory(matcher.map));
										for (MatcherSample sample : samples)
	                            			oldState.update(matcher.execute(oldState.vector(), oldState.sample(), sample), sample);
										state.set(oldState);
									} catch (JSONException e) {
                        ...
                        response.append(/*result*/state.get().toJSON().toString());
